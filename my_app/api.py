...
from my_app.schemas.pred import ModelOutput
...
@api_router.get('/performance', response_model=schemas.PerformanceCheck, status_code=200)
def performance() -> dict:
    ...
    if not results['errors']:
        ...
        val = accuracy_score(
            test_df[config.model_config.target],
            preds
        )
...
def predict(*, input_data: schemas.MulitpleSpaceshipTitanicInput) -> Any:
    ...
    results = make_prediction(input_data=input_data.replace({np.nan: None}))
